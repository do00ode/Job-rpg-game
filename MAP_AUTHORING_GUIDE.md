...
Use that guide for ASCII rows, passability symbols, spawns, dialogue-driven encounter actors,
repeatable weighted random encounters, transitions, and the validation workflow. A map is now
playable through the one generic exploration scene: add only its validated JSON definition,
localization key, and transitions from existing map JSON.
